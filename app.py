...
APP_VERSION = "2026.06.06-r8-vercel-ready"
...
<div class="wrap">
  ...
  <section class="dashboard">
    <article class="summary-card"><h2>자료 업로드 현황</h2><div class="ratio"><span id="ratioText">0</span><small>%</small></div><div class="progress"><span id="ratioBar"></span></div><div class="summary-meta"><b id="submittedText">0 / 10개 부서 제출</b><br><span id="statusUpdated">최근 갱신: -</span></div></article>
  ...
  <div class="main"><nav class="side" id="deptNav"></nav><main class="content"><div class="section-title"><h2 id="deptTitle"></h2><button class="btn ghost" onclick="addItem()">+ 업무 추가</button></div><div class="hint">부서별 업무를 직접 입력하면 상단 현황판에 입력 여부와 취합비율이 즉시 반영됩니다.</div><div class="count"><div>현재 부서<b id="deptCount">0건</b></div><div>전체 업무<b id="totalCount">0건</b></div><div>입력 부서<b id="filledCount">0개</b></div></div><div id="items"></div></main></div>
...
</section><p class="notice">※ 데이터 초기화는 현재 세션의 입력 내용, 업로드 현황, 저장된 업로드 파일을 모두 빈 상태로 비웁니다. 현재 원본은 legacy HWP 형식이므로 입력값이 반영된 완전 동일 HWP 생성을 위해서는 원본 HWPX 변환 및 XML 매핑 단계가 필요합니다.</p></div>
<script>
const seed=__SEED__;
